...
- **Apache Maven**
- **Postman**
- **Microsoft SQL Server** and **SQL Server Management Studio** - Manual installations only!
...
### Step 7: Install Microsoft SQL Server AND Microsoft SQL Server Management Studio

Navigate to Microsoft's download page for SQL Server [here](https://www.microsoft.com/en-us/sql-server/sql-server-downloads)

Once you've made it to the website scroll down till you see the option for "Express", click "Download Now"

![Image of Options on SQL Server page](./images/sql-server-download.png)

Download the installer and open from your "Downloads" folder, you may be asked for permission to update your device, select "Yes" and a pop-up should appear that looks like the following

![Image on install options for SQL Server](./images/sql-server-options.png)

Select the basic option and accept the liscensing agreement. Click install to begin the installation process.

![Install wizard for SQL Server](./images/sql-server-install.png)

Once completed successfull, you should see a screen that looks like the following image. Make sure to take note of your Connection String as it could be helpful later

![Successful installation of SQL Server](./images/sql-server-success.png)

Now we need to install the SQL Server Management Studio (SSMS). This will allow us to interact with our SQL Server database in a nice, easy way. Click on the "Install SSMS" option to begin this process. Feel free to close the SQL Server install page now that it's done.

![Linking to download page for ssms](./images/ssms-install-begin.png)

Clicking the button will take us to the following page [here](https://learn.microsoft.com/en-us/sql/ssms/download-sql-server-management-studio-ssms?view=sql-server-ver16&redirectedfrom=MSDN). From here, we can scroll till we see the option for "Download SSMS"

![download ssms](./images/ssms-download.png)

This download file is a bit larger so it might take some time. 

Before we begin installing the SSMS, we'll need to restart our computer for some changes to take effect, so restart your computer and then come back to these instructions. Once it's done, open the downloaded file from your downloads folder. You'll see a page that looks like the following image. Click "Install" to begin the installation process. If asked for permission to make changes to your device, select "Yes"

![Actually beginning install now](./images/ssms-install.png)

You'll need to restart again after installation successfully completes. You should be all good now, you should be able to connect to your server using the "Windows Authentication" option. After that, it's ready for your to start creating queries and executing them!
